...
## What we will cover{visibility="hidden"}
...
## A disclaimer{visibility="hidden"}
...
# Our tools are meant for panel data

## [**Panel data**](https://en.wikipedia.org/wiki/Panel_data), or longitudinal data, contains measurements of subjects over time
...
```{r panel-ca-ex}
dv_versioned_panel_final |> filter(geo_value == "ca") |> select(-version) %>% as_epi_df()
```
...
# Our tools are specifically designed to handle versioned data
...
* So, a "Hospital admission" may not be attributable to a particular condition
...
## So we have to track both [when the event occurred]{.primary} and [when it was reported]{.primary}

* The event time is indicated by `time_value`

* And we add a second time index to indicate the data `version`

```{r versioned-ca-ex}
#| echo: false
dv_archive <- dv_versioned_panel |>
  as_epi_archive(compactify = TRUE)
dv_archive |> filter(geo_value == "ca")
```



## Latency and revision in signals{visibility=hidden}

* [**Revised**]{.primary} data is updated or corrected after initial publication
...
2. [**Predictable revisions**]{.primary}
...
3. [**Revisions that are large and erratic**]{.primary}
...
<small>For the HRR containing Charlotte, NC</small>
...
* [**DV-CLI signal (left)**]{.primary}: regularly revised, but effects fade

* [**JHU CSSE cases (right)**]{.primary} remain "as first reported" until a major correction is made on Oct. 19
...
# Access Epidata data with `epidatr`

## [**Epidata:**]{.primary} is a repository of aggregated epi-surveillance time series
...
## Goals of Delphi Epidata platform and repository{visibility=hidden}
...
## Features of Delphi Epidata{visibility="hidden"}
...
## Epidata Documentation{visibility=hidden}
...
## API key grants privileged access and can be obtained by [registering with us](https://api.delphi.cmu.edu/epidata/admin/registration_form)
...
hhs_flu_nc <- pub_covidcast(
  ...
  time_type = 'day',
  geo_values = 'nc',
  time_values = epirange("2025-05-01", "2024-04-14")
)
head(hhs_flu_nc)
```
...
## Fetching data - COVIDcast main endpoint{visibility="hidden"}
...
## Returned data - COVIDcast main endpoint{visibility=hidden}
...
## Geographic levels{visibility=hidden}
...
## COVIDcast main endpoint - Example query{visibility=hidden}
...
## Obtaining data "as of" a specific date{visibility=hidden}
...
## Versioning is important for forecasting{visibility="hidden"}
...
## Main takeaways{visibility=hidden}
