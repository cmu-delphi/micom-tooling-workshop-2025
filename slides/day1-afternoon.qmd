...
## Essentials of `{dplyr}` and `{tidyr}` {visibility="hidden"}

## Selecting columns with `select()`  {visibility="hidden"}
...
## Extracting columns with `pull()`  {visibility="hidden"}
...
## Subsetting rows with `filter()`  {visibility="hidden"}
...
## Combining `select()` and `filter()` functions  {visibility="hidden"}
...
## Using the pipe operator `|>`  {visibility="hidden"}
...
## Grouping data with `group_by()`  {visibility="hidden"}
...
## Creating new columns with `mutate()`  {visibility="hidden"}
...
## Combining `group_by()` and `mutate()`  {visibility="hidden"}
...
## Conditional calculations with `if_else()` {visibility="hidden"}
...
## Summarizing data with `summarise()` {visibility="hidden"}
...
## Using `count()` to aggregate data {visibility="hidden"}
...
## Key practices learned {visibility="hidden"}
...
## 1 to 2 word summaries of the `dplyr` functions {visibility="hidden"}
...
## Tidy data and Tolstoy {visibility="hidden"}
...
## What is tidy data? {visibility="hidden"}
...
## `pivot_wider()` and  `pivot_longer()` {visibility="hidden"}
<div style="text-align: center;">
...
## Making data wider with `pivot_wider()` {visibility="hidden"}
...
##  Tidying messy data with `pivot_longer()` {visibility="hidden"}
...
## Introduction to joins in `dplyr` {visibility="hidden"}
...
## Subset `cases_df` {visibility="hidden"}
...
## Load state census data {visibility="hidden"}
...
## Left Join: Keep all rows from the first dataset {visibility="hidden"}
...
## Right Join: Keep all rows from the second dataset {visibility="hidden"}
...
## Inner Join: Only keep matching rows {visibility="hidden"}
...
## Full Join: Keep all rows from both datasets {visibility="hidden"}
...
## Pictorial summary of the four join functions {visibility="hidden"}
...
## Final thoughts on joins {visibility="hidden"}
...
## Two review questions {visibility="hidden"}
...
# (Versioned) Panel Data Processing Functions
...
## Features - Systematic lag analysis {visibility="hidden"}
...
## Features -- sliding computation over `epi_archive`s
...
## Ratio nowcaster: jumping from provisional to finalized value {visibility="hidden"}
...
## Ratio nowcaster: building training samples {visibility="hidden"}
...
## Revisiting `revision_analysis()` {visibility="hidden"}
...
revision_data$revision_behavior |> select(geo_value, time_value, lag_near_latest) |> slice_sample(n = 5)
...
## Ratio nowcaster: test time feature {visibility="hidden"}
...
## Nowcasting at a single date: building training samples {visibility="hidden"}
...
## Nowcasting at a single date: estimating ratio model {visibility="hidden"}
...
## Nowcasting at a single date: test feature construction {visibility="hidden"}
...
## Nowcasting at a single date: producing the nowcast {visibility="hidden"}
...
## Nowcasting for multiple dates {visibility="hidden"}
...
## Nowcasting for multiple dates via `epix_slide()` {visibility="hidden"}
...
## Sanity check of `epix_slide()` {visibility="hidden"}
...
## Details of `epix_slide()` {visibility="hidden"}
...
## Visualize nowcasts {visibility="hidden"}
...
## Smoothing nowcasts {visibility="hidden"}
...
## Evaluation using MAE {visibility="hidden"}
...
## Nowcasting: Moving from one predictor to multiple {visibility="hidden"}
...
## Start with a single nowcast date {visibility="hidden"}
...
## What predictors were available at test time? {visibility="hidden"}
...
## Line up with training data {visibility="hidden"}
...
## Get multiple predictors {visibility="hidden"}
...
## Combine with target data {visibility="hidden"}
...
## Fit the regression model {visibility="hidden"}
...
## Our prediction {visibility="hidden"}
...
## Backtesting our nowcaster {visibility="hidden"}
...
## Some other modifications {visibility="hidden"}
...
## Comparison: linear regression {visibility="hidden"}
